...
export default function CreateLessonPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await createLesson(formData, token);
      
      if (result.success) {
        router.push("/admin/lessons");
      } else {
        setError(result.error || "Failed to create lesson. Please try again.");
      }
    } catch (err) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-indigo-200/70 mt-1">
          Add a new Python lesson to the platform
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <CardHeader>
            ...
            <CardDescription className="text-indigo-200/70">
              Fill out the information below to create your lesson
            </CardDescription>
          </CardHeader>
